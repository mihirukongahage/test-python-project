...
Provides validation, formatting, and helper functions for task management.
"""
...
def format_task_description(task: str, max_length: int = 50) -> str:
    """
    Format task description, truncating if necessary.
...
def calculate_task_age(task: Dict) -> int:
    ...
    Returns:
        Age in days, or -1 if calculation fails
    """
    try:
...
def calculate_statistics(tasks: List[Dict]) -> Dict[str, any]:
    """
    Calculate comprehensive statistics from task list.
    ...
    high_priority_pending = sum(1 for t in tasks 
                               if t.get('priority') == 'high' and not t.get('completed', False))
...
def reindex_tasks(tasks: List[Dict]) -> List[Dict]:
    """
    Reindex task IDs to be sequential starting from 1.
    
    Args:
        tasks: List of task dictionaries
    
    Returns:
        List of tasks with reindexed IDs
    """
...
def create_task(task_description: str, priority: str = 'medium', 
    ...
    Args:
        ...
        task_id: Task ID (optional, will be assigned if not provided)
...
def format_date(date_str: str, format: str = '%Y-%m-%d %H:%M') -> str:
    """
    Format an ISO date string to specified format.
    
    Args:
        date_str: ISO format date string
        format: Output format (default: '%Y-%m-%d %H:%M')
...
def is_overdue(task: Dict, threshold_days: int = 7) -> bool:
    """
    Check if a pending task is overdue.
    
    Args:
        task: Task dictionary
        threshold_days: Number of days before considering overdue
    
    Returns:
        True if task is pending and older than threshold
    """
...
def export_tasks_to_dict(tasks: List[Dict]) -> Dict:
    """
    Export tasks with metadata for backup/export.
...
def import_tasks_from_dict(data: Dict) -> List[Dict]:
    """
    Import tasks from exported dictionary.
